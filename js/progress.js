...
// the README pushes "Zum Home-Bildschirm" so hard.

const PROGRESS_KEY = (deckId) => `italiano:progress:v1:${deckId}`;
const SETTINGS_KEY = 'italiano:settings:v1';
// The streak spans every deck: practising food words keeps it alive just as
// well as the main list.
const STREAK_KEY = 'italiano:streak:v1';
 ...
 * or to 'hint' if part of the answer was uncovered before typing it.
 */
export function recordResult(progress, wordId, grade) {
  ...
  if (grade === 'again') {
    ...
    // Right, but with help: it counts, yet the interval must not stretch or the
    // word would stop coming round while it still cannot be recalled unaided.
    rec.correct += 1;
  } else {
    rec.correct += 1;
...
/** Words answered wrong at least a third of the time, worst first. */
...
/**
 * Local calendar day. Deliberately not UTC: a streak should follow the day the
 * user is living in, so practising at 11pm and again at 8am is two days.
 */
function dayKey(ts) {
  const d = new Date(ts);
 ...
 * counts, which is the honest reading of "practised today". Repeat calls on the
 * same day are no-ops.
 */
